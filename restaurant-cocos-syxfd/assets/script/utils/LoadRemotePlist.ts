...
// export default function (url: string, callback: (atlasSprite: cc.SpriteFrame[] | any) => void) {
...
//         texture = cc.path.dirname(url) + "/" + texture
//         cc.loader.load(texture, function (err, tex) {
//             if (err) {
//                 return callback(err);
//             }
//             let atlasSprite = parsePlist(plist, tex);
//             callback(atlasSprite);
//         });
//     });
// };

export default function (plistUrl: string, textureUrl: string, callback: (atlasSprite: cc.SpriteFrame[] | any) => void) {

    resLoader.loadRemoteRes(plistUrl, function (err, plist) {
        if (err) {
            return callback(err);
        }
        let _plist = plist._$nativeAsset
        // let texture = _plist.metadata.realTextureFileName || _plist.metadata.textureFileName;
        // texture = cc.path.dirname(plistUrl) + "/" + texture
        resLoader.loadRemoteRes(textureUrl, function (err, tex) {
            if (err) {
                return callback(err);
            }

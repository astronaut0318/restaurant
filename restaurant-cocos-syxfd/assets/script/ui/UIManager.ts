...
import { resLoader, ProcessCallback } from "../libs/res/ResLoader";
...
export class UIManager {
    ...
    private autoExecAnimation(uiView: UIView, aniName: string, aniOverCallback: () => void) {
        ...
        //https://gitee.com/ifaswind/eazax-ccc/blob/master/components/popups/PopupBase.ts
        ...
        if (aniName == "uiClose") {
            ...
            cc.tween(main)
                .to(0.3, { scale: 0.5, opacity: 0 }, { easing: 'backIn' })
                .call(() => {
                    aniOverCallback();
                })
                .start();
    ...
    private getOrCreateUI(uiId: number, processCallback: ProcessCallback, completeCallback: (uiView: UIView) => void, uiArgs: any): void {
        ...
        if (null == uiPath) {
            _LOG(`getOrCreateUI ${uiId} faile, prefab conf not found!`);
        ...
        resLoader.loadRes(uiPath, processCallback, (err: Error, prefab: cc.Prefab) => {
            // 检查加载资源错误
            if (err) {
                _LOG(`getOrCreateUI loadRes ${uiId} faile, path: ${uiPath} error: ${err}`);
                completeCallback(null);
                return;
            }
            // 检查实例化错误
            ...
            if (null == uiNode) {
                _LOG(`getOrCreateUI instantiate ${uiId} faile, path: ${uiPath}`);
                completeCallback(null);
            ...
            if (null == uiView) {
                _LOG(`getOrCreateUI getComponent ${uiId} faile, path: ${uiPath}`);
                uiNode.destroy();
                completeCallback(null);
                resLoader.releaseAsset(prefab);
                return;
     ...
     */
    private onUIOpen(uiId: number, uiView: UIView, uiInfo: UIInfo, uiArgs: any, openAni) {
        if (null == uiView) {
            return;
        }
        // 激活界面
        ...
        this.autoExecAnimation(uiView, openAni, () => {
    ...
    public preload(uiId: number, onComplete?: (items: []) => void): void {
        let prefab = this.UIConf[uiId].prefab;
        cc.resources.preload(prefab, (error: Error, items: []) => {
            if (error) {
                console.error("预加载" + prefab + "失败");
            } else {
                console.log("预加载" + prefab + "成功");
                if (onComplete) {
                    onComplete(items);
                }
            }
    ...
    /** 打开界面并添加到界面栈中 */
    public open(uiId: number, uiArgs: any = null, progressCallback: ProcessCallback = null, openAni = null): void {
        let uiInfo: UIInfo = {
            uiId: uiId,
            uiArgs: uiArgs,
            uiView: null,
        ...
        this.getOrCreateUI(uiId, progressCallback, (uiView: UIView): void => {
            ...
            // 打开UI，执行配置
            this.onUIOpen(uiId, uiView, uiInfo, uiArgs, openAni);
            this.isOpening = false;
            this.autoExecNextUI();
        }, uiArgs);
    }
    ...
    public showWindow(uiId: number, uiArgs: any = null, progressCallback: ProcessCallback = null) {
        // this.open(uiId, uiArgs, progressCallback, "uiOpen")
    ...
    public close(closeUI?: UIView) {
        ...
        let close = () => {
            ...
            if (uiView.cache) {
                ...
                _LOG(`uiView removeFromParent ${uiInfo.uiId}`);
            } else {
                uiView.node.destroy();
                _LOG(`uiView destroy ${uiInfo.uiId}`);
            }
            this.autoExecNextUI();
        }
        // 执行关闭动画
        ...
        this.autoExecAnimation(uiView, closeAni, close);
    }
    ...
    public resize() {
        ...
        let cvs = cc.find('Canvas').getComponent(cc.Widget);
...
export let uiManager: UIManager = UIManager.inst